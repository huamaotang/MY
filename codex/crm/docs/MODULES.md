...
本文是整个仓库的架构事实来源。具体命令在 [开发手册](manuals/README.md)，接口、JSON 模型、配置和表结构分别在 [API 参考](reference/API.md)、[API 数据模型](reference/API_MODELS.md)、[配置参考](reference/CONFIGURATION.md) 与 [数据库参考](reference/DATABASE.md)。生产评审还必须阅读 [已知限制](reference/KNOWN_LIMITATIONS.md)。
...
| 调度边界                     | Prefect 是仓库当前唯一调度面；不要再并行注册 crontab/timer 运行同一业务任务 |
