...
本文记录当前代码真实存在的接口。REST 事实来源是 Java Controller 和 `deploy/nacos/gateway-dev.yaml`；客户端覆盖来自 Web `src/api.ts`、iOS `ApiClient.swift` 和 Android `ApiClient.java`。字段、类型与完整 JSON 示例见 [API 数据模型](API_MODELS.md)。
...
当前删除实现不会级联联系人、跟进、商机、合同或回款；生产使用前必须阅读 [已知限制](KNOWN_LIMITATIONS.md) 并完成关联数据策略。
...
基金删除当前只清理用户自选和 `fund_detail`，不会级联净值、评分、用户持仓等关联数据，不能把它当作完整数据销毁能力。
...
| ---- | ------------------------------------------- | ------------ | ---------------------------------------------------------------------------------------- |
| POST | `/api/portfolio/imports/ocr`                | multipart    | `sourceLabel=alipay/tencent`；`importType=holding/trade`；1～3 个 JPG/JPEG/PNG `images`；返回预览 |
...
限制：单文件 10 MB、单请求 20 MB，Java 等待 OCR 子进程最多 120 秒。解释器、脚本路径、确认请求和完整响应字段见 [配置参考](CONFIGURATION.md) 与 [API 数据模型](API_MODELS.md)。
...
| `feature`         | 标准差、夏普比率      | `fund_feature_data`                           | 08:00 最久未刷新优先分批 |
...
| `score`           | 快照、评分、回测、任务队列 | `fund_score_*`                                | 10:00、22:30 pipeline  |
...
| `score-pipeline`          | `fund-score-pipeline`  | 每日 10:00、22:30 | 已启用  |
