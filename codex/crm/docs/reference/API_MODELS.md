...
请求必须恰好包含当前 23 个因子 key（其中 `decline_today` 为当日预估跌幅，来自持仓与当日行情估算；`decline_1d/1w/2w/3w/4w` 为净值窗口短期跌幅），上述跌幅因子均为"跌幅越大分值越高"，每项为 0～100 的整数且总和为 100。约束由 Java 与 `scoring.py` 共同校验，不要仅靠 UI 拼装未知 key。
