...
没有数据库外键并不代表没有业务关系。当前 Customer Service 删除客户时只删除主表行，不会清理联系人、跟进、商机、合同等关联；生产启用删除前必须实现并测试明确的拒绝、归档或事务级联策略。
...
仓库当前没有自动归档/清理任务。上线前必须由业务、安全和运维确定：

| 数据 | 增长来源 | 必须决定 |
| --- | --- | --- |
| `sys_api_log` | 每次请求审计 | 在线保留期、归档、脱敏和清理批次 |
| 净值/业绩/行情/评分 | Python 周期任务 | 永久/分层保留、分区和查询性能 |
| 资讯 JSON | 高频资讯任务 | 数据授权、压缩/归档和保留期 |
| OCR 批次/文本/哈希 | 用户上传与确认 | 隐私、用户删除、法律留存和审计 |
| Prefect 元数据 | Flow/Task/日志 | Server 数据库保留和清理窗口 |

OCR 原始图片写入临时文件并在请求结束时尝试删除；识别文本、警告、哈希和批次会保存在 MySQL。清理实现必须先 dry-run、限制批量、记录影响行数，并在备份副本演练。
